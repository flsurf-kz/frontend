<script lang="ts">
    ...
    import { ContractEntityStatus, type ContractEntity, type Money, type UserEntity, type FileEntity, UserEntityType } from 'flsurf-client';
    ...
    const isFreelancerView = $CurrentUser?.type === UserEntityType.Freelancer;
    ...
    let isFreelancer = $CurrentUser?.type === UserEntityType.Freelancer; 
    ...
    <div class="space-y-6">
        {#if data.contracts && data.contracts.length > 0}
            {#each data.contracts as contract (contract.id)}
                ...
                {@const peerUser = isFreelancerView ? contract.employer   : contract.freelancer}
                {@const peerRole = isFreelancerView ? 'Заказчик'          : 'Исполнитель'}
                {@const title      = contract.job?.title
                                   ?? `Контракт от ${formatDate(contract.createdAt)}`}
                <a  href={`/contracts/${contract.id}`}
                    class="block card bg-base-100 shadow-lg hover:shadow-xl transition-shadow
                           rounded-xl border border-transparent hover:border-primary/40">

                    <div class="card-body p-5 md:p-6">
                        <div class="flex flex-col md:flex-row gap-4">

                            <!-- левая колонка: контрагент -->
                            <div class="flex-shrink-0 md:w-1/4 flex items-center md:flex-col md:items-start
                                        text-center md:text-left mb-3 md:mb-0 md:border-r
                                        md:border-base-200 md:pr-4">

                                {#if peerUser}
                                    <UserAvatar avatarFile={peerUser.avatar}
                                                className="w-12 h-12 md:w-16 md:h-16 rounded-full
                                                           mb-0 md:mb-2 mr-3 md:mr-0"/>
                                    <div class="flex-grow">
                                        <p class="font-semibold text-base-content text-md leading-tight">
                                            {peerUser.fullname || '—'}
                                        </p>
                                        <span class="text-xs text-base-content/60">{peerRole}</span>
                                    </div>
                                {:else}
                                    ...
                                    <p class="font-semibold text-base-content">Пользователь не указан</p>
                                {/if}
                            </div>
                            ...
                            <div class="flex-grow">
                                ...
                                <div class="flex justify-between items-start mb-1">
                                    <h3 class="text-lg md:text-xl font-semibold text-base-content
                                               hover:text-primary transition-colors line-clamp-2">
                                        {title}
                                    </h3>

                                    <span class="badge {statusInfo.className} badge-md !font-semibold
                                                   whitespace-nowrap">
                                        {statusInfo.text}
                                    </span>
                                </div>
                                ...
                                <div class="text-sm text-base-content/80 space-y-0.5">
                                    ...
                                    <p class="text-xs text-base-content/60">
                                        ID: <span class="font-mono">{contract.id.slice(0, 8)}…</span>
                                    </p>
                                </div>
