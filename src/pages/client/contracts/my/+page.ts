...
export const load: PageLoad<MyClientContractsPageData> = async ({ url }) => {
    ...
    // @ts-ignore - если UserEntity не имеет поля role или оно опционально
    if (currentUser.role?.toString().toLowerCase() !== 'client' && currentUser.type?.toString().toLowerCase() !== 'client') {
        console.warn(`Доступ к client/contracts/my запрещен для пользователя ${currentUser.id} с ролью ${currentUser.role || currentUser.type}.`);
        // Можно показать ошибку доступа или редиректнуть на главную
        // Для примера, если это не клиент, но он залогинен, можно редиректнуть на его дашборд
        throw redirect(307, '/'); 
    }
